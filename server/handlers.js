...
// Could also be used in future: get single company ingi
...
const getProductsByCategory = (req, res) => {
  ...
  const productsBySelectedCategory = products.filter(
...
const getProductsByCompany = (req, res) => {
  ...
  const productsByCompanyArray = products.filter(
...
// Update stock once an order purchase is complete
const updateProductStock = (req, res) => {
  ...
  if (productBody.item.name === oldProduct.name) {
    ...
    const updatedProduct = {
      ...oldProduct,
      numInStock: oldProduct.numInStock - parseInt(productBody.item.numInCart),
    };
